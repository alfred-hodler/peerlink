...
impl peerlink::Message for Msg {
    const MAX_SIZE: usize = 1 + 4 + 1024 * 1024;
...
fn sender(mut args: pico_args::Arguments) -> Result<(), Error> {
    ...
    eprintln!("sender {}: binding to {}", index, bind_addr);

    let handle = peerlink::run::<_>(Config {
        bind_addr: vec![bind_addr.into()],
        stream_config: peerlink::StreamConfig {
            tx_buf_min_size: Msg::MAX_SIZE,
            tx_buf_max_size: MaxMessageSizeMultiple::FOUR,
    ...
    let bytes: bytes::Bytes = vec![1; size as usize].into();
    ...
    loop {
        match handle.recv_blocking().unwrap() {
            ...
            Event::QueueRejected { .. } => {
                eprintln!("sender {}: queue rejected at count {}", index, remaining);
                unreachable!("queue rejected?")
            }
            Event::Message {
                message: Msg::Done, ..
            } => {
                eprintln!("sender {}: received done signal", index);
                break;
            }
            _ => {}
        }
    ...
    eprintln!("sender {}: sent {} messages", index, actual_sent);
    handle
    ...
    loop {
        match handle.recv_blocking().unwrap() {
            Event::ConnectedTo { result, target } => {
                ...
                peers.push(peer_id);
                eprintln!("sink: connected to target={} peer_id={} ", target, peer_id);
    ...
    loop {
        match handle.recv_blocking().unwrap() {
            ...
            Event::Disconnected { peer, reason } => {
                eprintln!("peer {} disconnected: {:?}", peer, reason);
            }
...
impl BenchMetrics {
    ...
    fn print_report(
        ...
        println!("{:-^60}", " THROUGHPUT BENCHMARK REPORT ");
