...
impl Connection {
    ...
    /// Encountering an error means that the connection must be discarded.
    ...
    ///
    /// Returns whether more data remains queued for writing. Encountering an error here means
...
impl Manager {
    ...
    /// Gets the connectedness status of a connection. For new connections, it attempts to
    /// put them in a usable state and register them with the registry.
    ///
    /// SAFETY: must be called only on tokens which are known to have a connection.
    pub fn try_ready<'a>(
        &'a mut self,
        token: &Token,
        registry: &Registry,
    ...
    /// Performs internal housekeeping on individual connections.
